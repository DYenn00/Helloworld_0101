# 🧠 Programming Fundamentals

Welcome to the **Programming Fundamentals** section of HelloWorld_0101!

If you're new to coding or want to strengthen your foundation, you're in the right place. This section covers the essential building blocks of programming, with beginner friendly explanations, examples, and mini projects.

---

## 📘 What You'll Learn

- ✅ Variables & Data Types
- ✅ Conditional Statements (`if`, `else`)
- ✅ Loops (`for`, `while`)
- ✅ Functions and Scope
- ✅ Arrays & Objects
- ✅ Error Handling
- ✅ Input & Output
- ✅ Basic Debugging

We break down each topic with:
- Simple definitions  
- Code examples (in Java, JavaScript and/or Python)  
- Real-world use cases  
- Mini challenges to test your understanding

---

## 📚 Posts & Lessons

- [What Are Programming Fundamentals?](./what-are-programming-fundamentals.md)
- [Understanding Variables](./understanding-variables.md)
- [Loops Made Easy](./loops-made-easy.md)
- [Loops Made Easy (Java)](./loops-made-easy-java.md)
- [Writing Your First Function](./writing-your-first-function.md)
- [Intro to Arrays](./intro-to-arrays.md)
- [Hello World 0101](./hello-world-0101.md)

> 📝 *More lessons coming soon. Check back regularly!*

---

## 🧑‍💻 Who This Is For

This section is great for:
- 🆕 Complete beginners
- 🧑‍🎓 Students 
- 🔁 Self-taught coders brushing up
- 🧱 Anyone who wants a strong programming base

---

## 🌱 Ready to Learn?

Choose a topic above and start exploring. Don’t forget to practice as you go.

Happy coding! 💻✨

---

✍️ *Created by D.Yenn*  
📅 *Last updated: October 1, 2025*

